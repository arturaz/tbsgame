...
case class Inverter[State](child: BehaviourTree[State]) extends DecoratorNode[State] {
  override def run(state: State) = child.run(state) match {
...
case class Succeeder[State](child: BehaviourTree[State]) extends DecoratorNode[State] {
  override def run(state: State) = (NodeResult.success, child.run(state)._2)
}

case class Failer[State](child: BehaviourTree[State]) extends DecoratorNode[State] {
  override def run(state: State) = (NodeResult.failure, child.run(state)._2)
}
...
case class RepeatUntilFail[State](
  ...
  @tailrec final override def run(state: State) = {
